...
// Component for procedural brain model
function BrainModel({ scrollRef }: { scrollRef: React.MutableRefObject<number> }) {
  ...
  const [purplePositions] = useState(() => {
    ...
    for (let i = 0; i < particleCount / 2; i++) {
      ...
      // Slightly tighter radius for core structure
  ...
  useFrame((state) => {
    ...
    if (brainRefPurple.current) {
      // Rotate opposite direction slightly for visual interest
...
// Orbiting particles reacting to scroll
...
// Main Canvas wrapper with fallback
export default function HeroScene() {
  ...
  if (!mounted) {
    // SSR Fallback (Static cybernetic network)
    return (
      <div className="w-full h-full flex items-center justify-center relative">
        <div className="w-64 h-64 rounded-full border border-dashed border-neon-cyan/20 animate-[spin_12s_linear_infinite]" />
        <div className="absolute w-48 h-48 rounded-full border border-dotted border-neon-purple/30 animate-[spin_8s_linear_infinite_reverse]" />
      </div>
    );
  }

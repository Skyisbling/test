...
function WaveMesh({ mouse }: { mouse: React.MutableRefObject<{ x: number; y: number }> }) {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Calculate complex wave height deforming in 3D
      // Mouse X controls wave frequency, Mouse Y controls amplitude distortion
    ...
    // Alert Three.js that vertices changed
    posAttr.needsUpdate = true;

    // Rotate mesh slowly for additional 3D depth
    if (meshRef.current) {
  ...
  return (
    <mesh ref={meshRef} rotation={[-Math.PI / 2.8, 0, 0]}>
      {/* High-density grid for smooth deformation */}
...
export default function SoundWaveMesh() {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Update ref to be read in useFrame
    ...
    return (
      <div className="w-full h-full flex items-center justify-center opacity-25">
        <div className="flex space-x-2">
          ...
          <div className="w-2 h-24 bg-neon-cyan rounded animate-pulse [animation-delay:0.2s]" />
          <div className="w-2 h-32 bg-neon-pink rounded animate-pulse [animation-delay:0.4s]" />
          <div className="w-2 h-20 bg-neon-purple rounded animate-pulse [animation-delay:0.6s]" />
          <div className="w-2 h-10 bg-neon-cyan rounded animate-pulse [animation-delay:0.8s]" />
        </div>
      </div>
    );
  }
